...
const log = getLogger('cli.scan-all');
...
async function main(): Promise<void> {
  ...
  log.info({ durationMs: durationMs ?? 'unbounded' }, 'starting both observatories');

  const cex = new CexScanOrchestrator();
  const poly = new PolymarketScanOrchestrator();
  const opts: { durationMs?: number } = {};
  if (durationMs !== undefined) opts.durationMs = durationMs;

  const [cexRun, polyRun] = await Promise.all([cex.run(opts), poly.run(opts)]);
  log.info(
    { cex: { id: cexRun.runId, status: cexRun.status }, poly: { id: polyRun.runId, status: polyRun.status } },
    'both observatories finished',
  );
}
