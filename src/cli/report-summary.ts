...
async function main(): Promise<void> {
  ...
  lines.push(`Raw cross-exchange candidates:       ${cex.totalCandidates}`);
  lines.push(`Approx. net-positive after fees:     ${cex.candidatesNetPositiveAfterFees}`);
  lines.push(`Depth-supported @ $100:              ${cex.depthSupportedAt100}`);
  lines.push(`Depth-supported @ $500:              ${cex.depthSupportedAt500}`);
  lines.push(`Tradable-under-prefunded:            ${cex.tradablePrefunded}`);
